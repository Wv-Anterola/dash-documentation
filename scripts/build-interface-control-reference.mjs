...
[
  ...
  },
  {
    label: 'Tab minimap',
    file: tabDocViewPath,
    needle: 'icon={<FontAwesomeIcon icon="globe-asia" size="lg" />}',
    tooltip: 'Show a minimap of this canvas',
    controlType: 'ToggleButton',
    beginner: 'Opens a small overview of the whole canvas in the corner of the tab, so you can see where you are in something larger than the screen.',
    visibility:
      'Appears only on the active tab of a freeform collection rendered with its normal layout. It is absent on key/value tabs, on other collection types, and on inactive tabs.',
    predicate:
      "tabView()?._isActive && !keyValue && doc.layout === CollectionView.LayoutString(...) && doc._type_collection === CollectionViewType.Freeform",
    handlerExpression: 'Popup(showUntilToggle) → TabMinimapView.popup → CollectionFreeFormView at _miniMapSize',
    technicalDetail:
      'The minimap is a second, non-interactive CollectionFreeFormView of the same document, rendered at _miniMapSize (150 by default) with a style provider that replaces each child with a flat colored block keyed to its document type.',
  },
  {
    label: 'Minimap viewport',
    file: tabDocViewPath,
    needle: 'className="miniOverlay" onPointerDown={this.miniDown}',
    tooltip: 'Drag to pan the canvas',
    controlType: 'Drag target',
    interaction: 'Press inside the minimap and drag.',
    beginner: 'The pale rectangle in the minimap is the part of the canvas you can currently see. Drag it to move around.',
    visibility: 'Appears inside the minimap once it has been opened.',
    handlerExpression: 'setupMoveUpEvents → doc._freeform_panX / _freeform_panY = clamp(...)',
    technicalDetail:
      'The drag delta is scaled by the rendered content bounds and clamped to them, and writes the pan fields directly on the collection document. It is not undoable: no batch is started, so panning by minimap cannot be reversed with undo.',
  },
  {
    label: 'Tab spring-loading',
    file: tabDocViewPath,
    needle: 'tab.element[0].onmouseenter = () => {',
    tooltip: 'Hover a tab while dragging to bring it forward',
    controlType: 'Hover target',
    interaction: 'Hold a drag over a tab without dropping.',
    beginner: 'While you are dragging something, hovering over a tab brings that tab to the front so you can drop onto it. Hovering without a drag just highlights the document elsewhere in the interface.',
    visibility: 'Every tab responds to hover; the tab switch happens only during a drag.',
    predicate: 'SnappingManager.IsDragging && tab.contentItem !== tab.header.parent.getActiveContentItem()',
    handlerExpression: 'setActiveContentItem(tab.contentItem) → tab.setActive(true) → Doc.BrushDoc(document)',
    technicalDetail:
      'The brush call is what makes the same document light up in the sidebar, the tree view, and on other canvases; the matching mouseleave unbrushes it. The tab switch is applied outside an undo batch.',
  },
  {
    label: 'Tab right-click',
    file: tabDocViewPath,
    needle: 'tab.element[0].oncontextmenu = (e: MouseEvent) => {',
    tooltip: 'Open the document’s menu from its tab',
    controlType: 'Click target',
    interaction: 'Right-click the tab.',
    beginner: 'Right-clicking a tab gives you the document’s own menu, the same one you would get by right-clicking the document itself.',
    visibility: 'Available on every tab whose content has rendered.',
    handlerExpression: 'getChild() → simulateMouseClick(child, clientX, clientY + 30)',
    technicalDetail:
      'There is no separate tab menu. Dash walks down to the deepest rendered content element and synthesises a click on it, offset 30 pixels down, so the document’s own context menu opens clear of the tab bar. The original event is stopped and its default prevented, which is what suppresses the browser menu.',
  },
  {
    label: 'Tile header right-click',
    file: dockingPath,
    needle: "if (dashboard && e.target === stack.header?.element[0] && e.button === 2)",
    tooltip: 'Add a new tab from empty header space',
    controlType: 'Click target',
    interaction: 'Right-click the empty part of a tile header, beside the tabs.',
    beginner: 'Right-clicking the blank strip next to a tile’s tabs adds a new empty canvas tab to that tile.',
    visibility: 'Only the header background responds. Right-clicking a tab in the same header opens that document’s menu instead.',
    predicate: 'e.target === stack.header.element[0] && e.button === 2',
    handlerExpression: "Docs.Create.FreeformDocument([], { title: 'Untitled Tab N' }) → inheritParentAcls → AddSplit(doc, OpenWhereMod.none, stack)",
    technicalDetail:
      'This path writes the new document straight into the header bar list and inherits the dashboard’s access control, but unlike the plus control it is not wrapped in an undo batch, so the new tab cannot be undone in one step.',
  },
  {
    label: 'Tab visibility icon',
    file: tabDocViewPath,
    needle: 'const closeIcon = <FontAwesomeIcon icon="eye" />;',
    tooltip: 'None: the element is never inserted into the page',
    controlType: 'Inert',
    interaction: 'None. It cannot be reached.',
    beginner: 'There is no eye control on a tab. An eye icon is built in the code but never placed on screen, so nothing to look for here.',
    visibility: 'Never visible: the element is constructed but never attached to the tab, so there is no state in which it appears.',
    handlerExpression: 'none',
    technicalDetail:
      'The icon is rendered into a detached div that is only ever stored on tab.reactComponents and recolored alongside the visible controls. Nothing in Dash-Web reads tab.reactComponents, and GoldenLayout does not know the property exists, so unlike its sibling iconWrap the element is never prepended to the tab. It is documented here so that reading the source does not suggest a control that users can find.',
  },
  {
