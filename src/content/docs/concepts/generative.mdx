...
Provider dependence is the first constraint. Firefly, OpenAI, and Stable Diffusion
...
<ProjectsForConcept concept="generative" />

<ProjectNote title="Image generation with style and structure references">
  The clearest case of the argument above: it uses documents already on the canvas as generation inputs. It is also unmerged, so the capability described here is not in the shipped application.
</ProjectNote>

<ProjectNote title="Selection-aware text formatting actions">
  Most of the work here went into the boundary handling rather than the prompting, which is a fair summary of what makes generative features usable or not.
</ProjectNote>

## Related documentation

- [Generative AI](/guides/features/generative-ai/) for how to use
  these features.
- [Ink](/guides/features/ink/) for drawing, which Smart Draw
  builds on.
- [Data visualization](/guides/documents/dataviz/) for
  chart generation from CSVs.
- [Agents and tools](/concepts/agents/) for models that
