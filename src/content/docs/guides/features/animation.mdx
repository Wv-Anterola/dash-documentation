---
title: Animation
description: 'The animation timeline: adding frames, edit mode, and pinning an animation into a presentation trail.'
---

:::caution[Developer mode]
Animation remains experimental. Open **Settings**, set **Mode** to
**Developer**, and work in a disposable or backed-up collection until you have
confirmed the behavior you need. Use undo promptly if an animation edit changes
more documents or frames than expected.
:::
...
![Screen recording: Stepping through animation frames and watching a document interpolate between them.](/assets/gifs/animation/basicanimation.gif)
...
![Screen recording: Adding and removing documents across animation frames, so they appear and disappear as the timeline plays.](/assets/gifs/animation/animateaddremove.gif)
...
![Screen recording: Editing keyframes directly on the animation timeline.](/assets/gifs/animation/editmode.gif)
...
![Screen recording: Switching the animation timeline into edit mode and back.](/assets/gifs/animation/editmodetransition.gif)

## Pinning to Presentations

The pin button next to the animation controls pins the open collection with the current view (read more about this on the [Trails](/guides/features/trails/) page). If you do this with an animation frame active, the presentation slide will now be associated with the animation frame. This means that you can pin a collection multiple times at different frames and use your animations in a presentation.

## Related

- [Presentation trails](/guides/features/trails/) for pinning the result into a
  trail.
- [Trails](/concepts/trails/) for what a trail is and where the idea comes from.
- [The spring animation model](/research/cohorts/2024/#animation-remodel-for-presentation-trails)
  for the 2024 project that replaced mechanical transitions with stiffness,
  damping, and mass.
