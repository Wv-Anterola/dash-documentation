...
<ProjectNote title="Desktop build with local model execution">
  The route that avoids sending documents to a hosted API: the desktop build can point the agent at a local Ollama instance instead. You install Ollama yourself, and the text features are the ones this covers; image and video generation still call external services.
</ProjectNote>
